function [histy, mflag] = MyHistSearch(SearchResults, bins, wr, histplot, nextnewfilt)
...
%   find the peaks in histograms  
    ...
    [ilcut iucut] = FindZeroPointCorrectionBound(N, X, wr);
...
if min(histy.Esample(2,:)) >= filt_n(1,1) && max(histy.Esample(2,:)) <= filt_n(1,2) && ...
        ...
        min(histy.ModLength(2,:)) >= filt_n(3,1) && max(histy.ModLength(2,:)) <= filt_n(3,2) &&...
        min(histy.Fit2R2(2,:)) >= filt_n(4,1) && max(histy.Fit2R2(2,:)) <= filt_n(4,2) &&...
        min(histy.Fit1AAR(2,:)) >= filt_n(5,1) && max(histy.Fit1AAR(2,:)) <= filt_n(5,2) &&...
        min(histy.Fit1MAR(2,:)) >= filt_n(6,1) && max(histy.Fit1MAR(2,:)) <= filt_n(6,2) &&...
        min(histy.Fit2AAR(2,:)) >= filt_n(7,1) && max(histy.Fit2AAR(2,:)) <= filt_n(7,2) &&...
        min(histy.Fit2MAR(2,:)) >= filt_n(8,1) && max(histy.Fit2MAR(2,:)) <= filt_n(8,2) &&...
        min(histy.hchange(2,:)) >= filt_n(9,1) && max(histy.hchange(2,:)) <= filt_n(9,2) &&...
        min(histy.Pchange(2,:)) >= filt_n(10,1) && max(histy.Pchange(2,:)) <= filt_n(10,2) &&...
        min(histy.Fit4AAR(2,:)) >= filt_n(11,1) && max(histy.Fit4AAR(2,:)) <= filt_n(11,2) &&...
        min(histy.Fit4MAR(2,:)) >= filt_n(12,1) && max(histy.Fit4MAR(2,:)) <= filt_n(12,2) &&...
        min(histy.P_star(2,:)) >= filt_n(13,1) && max(histy.P_star(2,:)) <= filt_n(13,2) &&...
        min(histy.h_star(2,:)) >= filt_n(14,1) && max(histy.h_star(2,:)) <= filt_n(14,2) &&...
        min(histy.dP(2,:)) >= filt_n(15,1) && max(histy.dP(2,:)) <= filt_n(15,2) &&...
        min(histy.dH(2,:)) >= filt_n(16,1) && max(histy.dH(2,:)) <= filt_n(16,2)
    mflag = 1;
end
